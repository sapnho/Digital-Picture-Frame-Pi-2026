...
NOTES = {
    ...
    "viewer.shrink_oversized": "What happens to a picture past that size. On, the frame opens it once in a separate process and keeps a screen-sized copy in its cache, so the photograph is shown from then on; the original is never moved, changed or deleted. Off leaves such a picture out of the slideshow.",
    "viewer.max_decode_megapixels": "The largest picture the frame will open. A photograph from a camera or a phone is scaled down while it is decoded and never reaches this; a huge PNG or TIFF scan cannot be, and would take the frame down with it, so past this size it is skipped and the log says so. 0 turns the limit off.",
    ...
    "library.deleted_folder": "Where “Remove” moves a picture. Every removal is written to removals.jsonl in this folder — when it went, where it came from, and what it was. A removed picture also stays out of the slideshow if the file turns up again, under any name, from a sync or a backup; only “Show this again” on the Removed tab puts it back in. That tab reads the journal, can put a picture back, and can empty the trash: that deletes the files and keeps the lines about them.",
...
LABELS = {
    ...
    "max_decode_megapixels": "Largest picture to open (MP)",
    "shrink_oversized": "Shrink a too-large picture instead of skipping it",
...
LIMITS: dict[str, tuple[float | None, float | None]] = {
    ...
    "viewer.max_decode_megapixels": (0.0, 1000.0),
...
ADVANCED = {
    ...
    "viewer.max_decode_megapixels", "viewer.shrink_oversized",
